...
// create a new kernel_pagetable for specific process
pagetable_t
...
void proc_kpagetable_freewalk(pagetable_t pagetable)
{
  ...
  for (int i = 0; i < 512; i++)
  {
    ...
    if (pte & PTE_V)
    {
      ...
      if ((pte & (PTE_R | PTE_W | PTE_X)) == 0)
        ...
        proc_kpagetable_freewalk((pagetable_t)PTE2PA(pte));
    }
  }
...
// Copies the page table but not the
// physical memory.
int pagetable_copyin(pagetable_t upagetable, pagetable_t kpagetable, uint64 start_sz, uint64 end_sz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  if (start_sz >= PLIC || end_sz >= PLIC)
    panic("pagetable_copyin: sz too big");

  for (i = start_sz; i < end_sz; i += PGSIZE)
  {
    if ((pte = walk(upagetable, i, 0)) == 0)
      panic("pagetable_copyin: pte should exist");
    if ((*pte & PTE_V) == 0)
      panic("pagetable_copyin: page not present");
    pa = PTE2PA(*pte);
...
err:
  uvmunmap(kpagetable, start_sz, i / PGSIZE, 0);
  return -1;
}
